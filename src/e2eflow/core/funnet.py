...
def funnet(flow):
    ...
    with tf.variable_scope('funnet') as sc:
        ...
        net = slim.conv2d(net, 1024, [1, 1], biases_initializer=tf.constant_initializer(0.001),
                          weights_initializer=trunc_normal(0.005), scope='fc7')
        end_points[sc.name + '/fc7'] = net

        net = slim.conv2d(net, 10, [1, 1], biases_initializer=tf.constant_initializer(0.001),
                          weights_initializer=trunc_normal(0.005), scope='fc8')
