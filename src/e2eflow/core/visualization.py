...
def get_flow_visualization(flow_tf):
    batch_size, h, w, n = flow_tf.shape.as_list()
    ...
    if n > 2:
        valid = flow_tf[:, :, :, 2]
    max_flow = tf.reduce_max(tf.maximum(du, dv))
    # angle layer
    # flow_viz[:, :, :, 0] = tf.scalar_mul(1. / (2. * PI), tf.atan2(dv, du))
    flow_viz0 = tf.atan2(dv, du)
    flow_viz0 = tf.scalar_mul(1. / (2. * PI), flow_viz0)
    # magnitude layer, normalized to 1
    flow_viz1 = tf.scalar_mul(8. / max_flow, tf.sqrt(tf.multiply(du, du) + tf.multiply(dv, dv)))
    # phase layer
    flow_viz2 = 8. - flow_viz1
    ...
    if n > 2:
        ...
        flow_viz[:, :, :, 0] = tf.multiply(flow_viz[:, :, :, 0], valid)
        flow_viz[:, :, :, 1] = tf.multiply(flow_viz[:, :, :, 1], valid)
        flow_viz[:, :, :, 2] = tf.multiply(flow_viz[:, :, :, 2], valid)
    ...
    # # taken from https://github.com/sampepose/flownet2-tf/blob/master/src/flowlib.py
    # # Author: Ruoteng Li
    # # Date: 6th Aug 2016
    # # ==============================
    # """
